...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    ...
    } else if ([segue.identifier isEqualToString:@"segueEdit"]) {
        FormDataViewController *formView = segue.destinationViewController;
...
- (void)onSavedDataBeacon:(MFData *)beacon isNewData:(BOOL)isNewData {
    if (isNewData) {
        ...
        // reload table data
        [self.tableView beginUpdates];
        [self.tableView reloadSections:[NSIndexSet indexSetWithIndex:self.tableView.indexPathForSelectedRow.section]
                      withRowAnimation:UITableViewRowAnimationAutomatic];
        [self.tableView endUpdates];
    }
}
